...
Feature layout - raw bytes, not typed lists:
    ...
    labels  bytes  uint8[OUTPUT_STEPS]        (4 classes fit in a byte)

`float_list` stores every sample as a separate protobuf field. At 3 leads that is 7,500
fields per example, and both writing and `parse_example` pay per field; one `bytes` feature
is a single memcpy each way. It is also smaller on disk: 30,500 bytes per example against
the ~38,000 the typed lists cost with their tags and lengths. The labels shrink 8x by going
from int64 to uint8.
...
def _convert_batch(seg_file, out_dir):
    """One npy batch -> one tfrecord. Returns (name, count, class-step histogram)."""
...
def build_dataset(db_name):
    ...
    for split in ("train", "eval"):
        ...
        for seg_file in tqdm(batches, desc=f"{db_name} {split}",
            ...
            _, n, hist = _convert_batch(seg_file, out_dir)
            total += n
            counts += hist
        summary[split] = {'segments': total, 'files': len(batches),
                          'class_steps': counts.tolist()}
...
def write_manifest(summary, merge=True):
    ...
    for split, v in totals.items():
        ...
        print(f"  {split:5s}: {v['segments']:>9,} segments | steps: {share}")
    return manifest
